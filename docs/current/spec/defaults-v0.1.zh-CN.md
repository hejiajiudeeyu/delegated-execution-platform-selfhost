...
| `request_event_scope_v0_1` | `ACKED_completed_failed` | FROZEN | v0.1 实现 `ACKED` 与卖家完成态观测事件 `COMPLETED/FAILED`，不实现 `RUNNING/PROGRESS` |
...
| `catalog_submission_mode` | `platform_onboarding_api_with_dual_admin_review` | FROZEN | 当前通过 `POST /v1/catalog/subagents` 提交 seller/subagent，上架前要求 seller 与 subagent 双审批 |
