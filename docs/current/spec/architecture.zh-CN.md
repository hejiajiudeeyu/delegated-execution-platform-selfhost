...
说明：
- L0 对 Buyer 暴露的控制面可观测主状态至少包括 `ACKED` 与 Seller 写入的 `COMPLETED/FAILED` 完成态；`RUNNING/PROGRESS` 仍不在 v0.1 事件范围内。
...
- `ACKED`：卖家 ACK 已记录
- `COMPLETED`：卖家成功结果已封包并完成控制面上报
- `FAILED`：卖家错误结果已封包并完成控制面上报
...
实现中还包含资源/运行时相关域，例如 `CATALOG_*`、`REQUEST_*`、`SELLER_*`、`USER_*`、`SUBAGENT_*`、`TRANSPORT_*`、`SIGNER_*`、`TASK_*`、`BUYER_*`、`RELAY_*`、`OPS_*`。
