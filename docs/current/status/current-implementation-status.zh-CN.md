...
说明：当前已具备三端可启动脚手架；并已补齐一版已实现基线联调闭环、协议参考控制面最小 RBAC、Buyer 验签信任链修正。
...
- [x] `POST /v1/catalog/subagents`（正式 seller/subagent onboarding 提交 API）
- [x] 目录导入流程最小版（平台 onboarding API + 版本记录 + seller/subagent 双审批）
...
- [x] `GET /v1/catalog/subagents/{subagent_id}`（目录详情接口，实现轻列表 + 详情分离）
- [ ] token claims 增加 `buyer_route_hash`，seller 侧做 return-route + claims 双因素校验
- [ ] 检索增强（联想检索、模糊搜索、领域策略、候选筛选）
- [ ] 目录快照/增量接口（`/snapshot`、`/changes`）
- [x] `POST /v1/admin/subagents/{subagent_id}/review-tests` + `GET /v1/admin/review-tests*`（隐藏审核测试最小链路）
- [ ] 其他 Buyer / Seller 外部 transport adapter（Email MCP / SMTP bridge / HTTP Webhook 的进一步扩展）
...
- [x] API 鉴权方式：API Key
- [x] 目录提交流程：`POST /v1/catalog/subagents` + admin 双审批 + 可选隐藏审核测试
