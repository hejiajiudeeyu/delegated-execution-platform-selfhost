...
它不是个人电脑终端用户 seller 安装的默认路径。

终端用户 seller 安装建议走 CLI 路径：
...
2. 设置 `PLATFORM_API_BASE_URL`、`PLATFORM_API_KEY`、`TRANSPORT_BASE_URL`、`SELLER_ID`、`SUBAGENT_IDS`
   - 仅当执行器与传输链路可安全并行执行任务时，才将 `SELLER_WORKER_CONCURRENCY` 设为大于 `1`
3. `docker compose up -d --build`
4. 检查 `http://127.0.0.1:${PORT:-8082}/healthz`

该 profile 以 SQLite 默认部署 `seller-controller` 独立服务，并依赖外部 relay。
