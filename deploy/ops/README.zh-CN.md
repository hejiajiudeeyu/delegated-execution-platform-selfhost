...
该路径会启动本地 supervisor，统一管理 buyer 与可选 seller；在 local transport 模式下，它会以独立进程启动 relay，而不是直接 import relay 源码。
运行日志写入 `~/.delexec/logs`，可在 `ops-console` 或 `debug-snapshot` 查看。
`ops-console` 现提供本地 buyer/seller onboarding 的 setup wizard，终端用户无需记忆完整步骤顺序。
...
Compose profile 行为：
- `buyer-controller` 始终开启
- `relay` 始终开启
- `seller-controller` 可选，仅在启用 `seller` profile 时启动

Docker Compose 快速开始：
以下步骤仅适用于 `deploy/ops` 下的 compose profile。
1. `cp .env.example .env`
2. 设置 `PLATFORM_API_BASE_URL`
3. 启动 buyer 模式：`docker compose up -d --build`
4. 后续启用本地 seller：`docker compose --profile seller up -d seller-controller`
...
3. 命令会写入 `~/.delexec/.env.local`：
   ...
   - `PLATFORM_API_KEY`
   - `BUYER_CONTACT_EMAIL`
4. 若 `buyer-controller` 已在运行，请重启使其加载新环境文件

说明：
- 注册 buyer 账户后请使用 `BUYER_PLATFORM_API_KEY`
- 面向用户的安装流程应优先统一 `ops` supervisor，而不是直接管理 `seller-controller`
- 目标体验是单一用户控制台：buyer 默认开启，seller 为可选角色
